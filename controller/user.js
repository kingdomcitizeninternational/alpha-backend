  ...
  try {
    ...
    for (const handler of handlers) {
      if (handler.daysLeft > 0) {
        ...
        if (handler.daysLeft === 0 && handler.depositsMade < handler.totalDepositsRequired) {
        ...
        const depositsRemaining = handler.totalDepositsRequired - handler.depositsMade;
        ...
        const emailHtml = `
          ...
          <p>Deposit amount: <strong>${handler.depositAmount} NGN</strong></p>
    ...
    return res.status(200).json({
      response: "Deposit handler updated and users notified successfully."
    });
  } catch (error) {
